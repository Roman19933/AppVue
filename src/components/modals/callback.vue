<template>
  <div>
    <b-modal id="callback" v-model="showModal" modal-class="custom" centered hide-header content-class="custom__content" dialog-class="custom__dialog" body-class="custom__body custom__body_callback" hide-footer no-stacking>
        <button @click="showModal = false" type="button" class="close custom__close" aria-label="Close">
            <svg class="icon-svg icon-svg-close-modal close-modal"><use xlink:href="../../assets/img/sprite.svg#close-modal"></use></svg>
        </button>
        ...
        <ValidationObserver ref="callbackV" v-slot="{ handleSubmit, invalid }">
            <form @submit.prevent="handleSubmit(onSubmit)">
                <div class="custom__body-right">
                    ...
                    <Button value="Заказать звонок" type="submit" :disabled="invalid" class="button button__modal callback-btn" v-b-modal.thank/>
                </div>
            </form>
        </ValidationObserver>
    </b-modal>
